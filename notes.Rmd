...
# pick up from here
replicating the mlr3fairness pipeline doesn't really make sense here
I could not dichotomize but keep all of the race groups and see if there are greater disparities --> the fairness adjustment won't be straight forward though
--> Do they work for non-binary PA?

text bothmann again for advice
...
The problem is that usual group fairness metrics do not show racial discrimination okay but the other papers would explain these results, it is just that i cannot show the things from the other papers on the data but only explain their reasoning
Did I really estimate the tpr on the target pop correctly? because it said that it would be HIGHER on the target population than on training - this basically cannot be
