...
- do same classifier for another grouping of race, and see if the picture is the same
for the current grouping in unprivileged (ppl of color) and privileged (other)
the measures are unfair towards the privileged group! But i think there are several problems with this:
1) we have way more data for the unprivileged groups
More data means better predictions
2) the data basically suffers from extreme selection bias because the fact that we have so many data from ppl of color is a representation of the unfairness itself
