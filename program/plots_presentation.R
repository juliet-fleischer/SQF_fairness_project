...
sample_dt |> 
  ...
  geom_segment(aes(x = min(PA_GROUP) - 0.3, xend = mean(PA_GROUP) - 0.3, y = 0.5, yend = 0.5), 
               linetype = "dashed") +
  geom_segment(aes(x = mean(PA_GROUP) + 0.3, xend = max(PA_GROUP) + 0.3, y = 0.6, yend = 0.6), 
               linetype = "dashed") +
  theme_scientific() +
  labs(
    x = "Geschützes Attribut",  # X-axis label
    y = expression(P(hat(Y) == 1)) # Y-axis mathematical expression
  ) +
  scale_x_continuous(
    breaks = c(0, 1),  # Specify where to place ticks
    labels = c("weiß", "schwarz")  # Specify what the tick labels should say
  ) +
  scale_color_manual(
    name = expression(hat(Y)),  # Legend title
    values = c("Y" = "#007A88", "N" = "#883A00"),  # Custom colors
    labels = c("Festnahme", "keine Festnahme")  # Custom labels
  )
