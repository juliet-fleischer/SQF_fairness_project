...
\begin{frame}[t]{Fairness anhand der Fehlermatrix konstruieren}
	...
	\begin{columns}
		...
		\begin{itemize}
			...
			\item<4-> z.B. Predictive Equality \cite{verma2018} $$P(\hat{Y} = 1 | A = a, Y = 0) = P(\hat{Y} = 1 | A = b, Y = 0)$$
			\item<5-> \textbf{Sufficiency:} Zuverlässigkeit der Vorhersage soll gleich sein
			\item<6-> z.B. Predictive Parity \cite{verma2018} $$P(Y = 1 | A = a, \hat{Y} = 1) = P(Y = 1 | A = b, \hat{Y} = 1)$$
		\end{itemize}
		\column{0.5\textwidth}
		\onslide<3->{
			\begin{center}
...
\begin{frame}{Ist die Gruppenzugehörigkeit der Grund für die Festnahme? \\ - kausale Definitionen}
    \begin{columns}
        ...
        \column{0.4\textwidth}
		\begin{itemize}
			\item Gruppen Fairness: FACE, FACT (on avverage or on conditional average level) \cite{Zafar2017PPNFC}
